...
发布器左侧的“DLC 时效检查”是独立页面。点击“检查全部 DLC 时效”可即时比对所有已配置游戏，结果以可滚动列表列出游戏、资源提交时间、Steam 最新 DLC 上线时间和状态。该检查只读取 Steam 与本地/发布输出的时间，不写入 AppInfo、不改动资源，也不会发布任何文件；每项标注“资源过时”“未过时”或“无法判断”。
