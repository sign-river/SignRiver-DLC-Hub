...
第二阶段提供了受限的补丁环境准备工具，只允许处理当前卡带 `patch_relative_dir` 下的补丁 DLL、原版备份 DLL 和 `cream_api.ini`，不会扫描、备份或修改 DLC 目录。使用顺序固定为：

1. 点击“记录补丁基线”，把这三个目标在测试前的存在状态、内容哈希和文件副本保存到当前验收轮次。
2. 在支持自动准备的补丁测试项目中选择具体方案。
3. 点击“预览环境准备”，确认将被暂时移除或替换的完整路径；预览不会修改文件。
4. 关闭游戏和客户端后点击“执行环境准备”。
5. 手动启动客户端完成测试。
6. 测试结束后点击“恢复测试环境”，严格按照基线还原原有文件，并移除测试期间新产生、但基线中原本不存在的目标文件。

当前自动方案包括：首次安装环境、当前 DLL 缺失、原版备份 DLL 缺失、INI 缺失以及当前 DLL 内容异常。安全软件隔离等无法可靠控制的场景仍保持人工操作。
