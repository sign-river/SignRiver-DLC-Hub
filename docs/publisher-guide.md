...
本地构建页会显示当前卡带的「资源提交时间」：取本地 `dlc/` 包与发布输出中最新修改时间，构建完成或生成卡带中心时自动刷新，并写入 `games/<id>/freshness.json`。卡带中心同时从当前 AppInfo 导出 Steam 最新 DLC 的正式上线时间；客户端在 DLC 列表上方保留提交时间，且仅当该上线时间严格更晚时提示资源可能未跟进，并提醒 UP 更新资源。旧卡带或 Steam 未提供日期时不显示该提醒。
...
- **发布中心**：创建、预检、确认、执行和恢复程序版本、游戏内容、Hub/公告发布批次；
- **内容与卡带**：准备游戏内容、维护卡带与公告，并创建对应批次；
- **验收中心**：记录人工验收结果，并把验收快照附加到批次；
- **高级维护**：保留远端删除、采用远端附件和单源发布等修复能力。

标准发布应优先使用“发布中心”的受控批次：

1. 创建对应类型批次并收集候选产物；
2. 运行机器预检，处理所有硬门禁；
3. 必要时在验收中心记录并附加人工验收快照；
4. 检查集中确认摘要，包括范围、指纹、双源目标、警告与阶段顺序；
5. 确认后执行，暂停或异常重启时从批次记录恢复；
6. 完成后查看远端回读、事件记录，必要时导出脱敏审计。

旧“构建与发布（兼容）”入口仅用于新路径尚未覆盖或真实验收中发现问题时回退。新旧路径不得同时对同一批次执行远端副作用。在真实双源演练、故障恢复演练和新旧 UI 对等验收全部通过前，不删除该兼容入口。新旧能力位置与回退边界见 `docs/publisher-compatibility-map.md`，正式验收步骤见 `docs/publisher-refactor-acceptance.md`。

高级维护操作必须先在发布中心选择或创建关联批次，并输入界面给出的完整确认文本。确认只代表授权；真实异步成功或失败后才写入最终审计结果。安全暂停后继续属于同一次授权，不可恢复失败后重新发起则必须重新确认。
