...
- [x] 多路径聚合 DLC：卡带可声明聚合扫描根，将跨多个目录分支的同名 DLC 合并发布；客户端按文件事务安装、识别、校验、回滚和安全卸载，不替换公共父目录。
...
- [x] “一键修复”：二次确认后先准备、校验补丁与全部 DLC 并预检磁盘空间，全部资源就绪后再清理旧内容、重置补丁、从缓存重装并做最终健康检查。
- [x] “一键移除补丁”：删除补丁 DLL 与 `cream_api.ini`，并把 `steam_api64_o.dll` 恢复为 `steam_api64.dll`。
- [x] “移除本程序安装内容”：只撤销本程序有安装凭据的 DLC，并恢复被覆盖前的同名内容；操作前检查游戏进程、未完成下载和原版 DLL 备份，游戏原有 DLC 与缓存不会被删除。
...
- [x] DLC 完整度检测：发布器对比 Steam 官方列表与本地卡带；结果写入客户端卡带并在 DLC 列表展示。
- [x] 远程游戏卡带主表：启动读取 `hub` Release / 本地 `config/cartridges`，按需下载单游戏卡带；发布器提供独立“卡带管理”页，统一生成完整主表并自动增量发布 `hub` Release。
...
- “一键修复”按钮先准备并校验全部资源和磁盘空间，再清理 DLC 与补丁、从缓存完整重装并最终复检；下载或预检失败时不先破坏现有游戏。
- “一键移除补丁”作为对称的破坏性操作与解锁并列，仅清理补丁三件套并把原版 DLL 恢复到位。
- “移除本程序安装内容”只撤销本程序有安装记录的 DLC、恢复被覆盖内容并安全还原补丁；不提供会批量删除卡带检测内容的危险入口，也不会删除可复用缓存。
