...
- [x] 下载状态机、整包重试、暂停/取消、大小与 SHA-256 校验、内容寻址缓存和坏包隔离。
- [x] SQLite 下载任务持久化、崩溃状态恢复、并发队列及速度/ETA 计算。
- [x] Stellaris Release 完整 DLC 滚动列表及逐项下载、暂停、继续和取消操作。
- [x] DLC 搜索、状态筛选、批量选择/下载及下载带宽限制策略。
- [ ] Range 来源续传、安装事务和其余完整 UI。
...
- [x] 严格 `dlc-catalog.json` v1 契约、签名验证接口、授权标志和安装信任门槛。
- [x] 本地 DLC ZIP 扫描与清单草稿生成工具（默认未授权、未签名）。
- [x] 用户界面安装管理接入：可信下载门槛、安装、后台检查、保守修复和安全卸载。
- [x] SQLite 用户设置、下载并发/限速配置、缓存与日志入口及最近日志预览。
- [x] 侧边导航与 DLC 库、下载任务、日志、设置、关于五个独立页面。
...
class ReleaseSource(Protocol):
    ...
    def get_manifest_asset(self, release: ReleaseRef) -> AssetRef: ...
...
| `DlcEntry` | `dlc_id, game_id, version, title, size, sha256, compatibility, requires, conflicts, artifacts` |
| `DownloadJob` | `job_id, artifact_id, expected_size/hash, state, bytes_complete, source_ref, retry/error` |
...
不能把新响应拼接到旧文件。完整包通过大小和 SHA-256 校验后才可进入包检查阶段。
...
- 每个 Release 可对应一个 DLC 版本，也可以承载同一目录修订下的多个资源。
- Release 中必须存在机器可读的 `dlc-catalog.json` 或约定的 manifest 资产。
- DLC 包使用稳定命名，例如 `<game_id>-<dlc_id>-<version>.zip`。
- 清单记录附件稳定 ID、包大小、SHA-256、兼容性、依赖和冲突；不只依赖文件名或 Release 描述。
- 同一 DLC/版本存在多个下载源时，只有 `size + sha256` 完全一致才能自动切换镜像。

### 6.2 目录清单 schema v1

顶层至少包含：

- `schema_version, catalog_id, revision, generated_at, expires_at`
- `publisher_id, key_id, channel, minimum_hub_version`
- `games[], dlcs[]`
- `signature`（正式发布阶段使用 Ed25519；签名绑定规范化后的清单内容）

每个 DLC 至少包含：

- 身份：`dlc_id, game_id, version, title, description`
- 兼容性：`game_version_range, platform, store, architecture, locale`
- 关系：`requires, conflicts, replaces, provides`
- 安装：逻辑安装根、相对子目录、安装策略、是否要求关闭游戏
- 资源：`artifact_id, format, size, sha256, unpacked_size, file_count, locations[]`

v1 只支持 ZIP；包内应包含 `files.manifest.json`，列出各文件的相对路径、大小和摘要。目录清单签名绑定包摘要，包摘要再绑定文件清单，形成完整验证链。
...
- 下载过程中限制响应总字节，超过清单 `size` 立即终止，提前 EOF 视为失败。
- 最终必须同时通过实际大小和 SHA-256；平台返回的 ETag 不能代替内容摘要。
- 摘要失败后将文件和脱敏元数据移动到 `cache/quarantine/`，禁止进入安装队列。
- 可更换摘要一致的镜像并有限次数重下；不能无限自动循环。
- 同一清单 revision 和 artifact digest 已被判定为结构性坏包时直接失败，直到清单修订或摘要改变。
- 隔离区设置容量和保留期，默认只保留最近的诊断样本。

### 7.4 下载任务持久字段

`job_id, game_id, dlc_id, dlc_version, artifact_id, manifest_revision/digest, expected_size/sha256, asset_refs, current_source, etag, last_modified, temp_path, bytes_complete, state, reason, attempt, next_retry_at, created_at, updated_at`。
...
- 外观：主题、语言、缩放。
- 下载：并发数、限速、重试预算、缓存目录、代理、自动继续。
- 游戏：每个游戏的路径、重新检测、忘记历史实例。
- 来源：GitLink 仓库、通道、镜像优先级、刷新间隔、凭据状态。
- 更新：稳定/测试通道、启动检查、自动下载应用更新。
- 存储与安全：缓存/隔离区清理、备份保留、签名和发布者信息。
- 关于：程序、启动器和 Host API 版本、许可证、项目主页、检查更新。
...
- 默认拒绝 localhost、私网、链路本地和非 HTTP(S) 下载地址，防止远端清单发起本地网络访问。
- 远端只能提供逻辑安装根和相对路径；拒绝绝对路径、`..`、盘符/UNC、ADS、Windows 保留名和尾随点/空格。
- 拒绝 ZIP path traversal、symlink/junction/reparse point、重复/大小写冲突路径、zip bomb 和异常压缩比。
- 严格限制压缩包大小、文件数、单文件/总展开量和目录深度。
- 清单签名正式启用前，来源只适合内部测试；SHA-256 不能防止清单和包同时被替换。
...
当前进度：核心单任务闭环、真实 GitLink 样例、SQLite 持久队列、并发控制、
