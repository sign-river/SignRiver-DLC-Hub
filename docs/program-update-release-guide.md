...
三端原生候选汇总到 Windows 后，优先在发布器“发布中心”创建**程序版本批次**，不要直接从兼容页逐个上传。批次流程会冻结三端文件的路径、大小、修改时间和 SHA-256，并在执行前再次核对；任一输入变化都会使原预检与确认失效。

标准顺序为：

1. 收集 Windows x64、SteamOS x64、macOS Intel x64 三端同版本候选；
2. 运行预检，确认平台、架构、包结构、版本、模块归档、更新说明和双源配置；
3. 附加必要的人工验收快照并查看集中确认摘要；
4. 两个源分别上传全部数据包并完成远端回读；
5. 只有双源数据包都验证通过后，才依次切换 GitLink、GitHub 清单；
6. 最终回读两个源的清单与附件，批次才可标记完成。

暂停、重启恢复和部分源失败都应从同一批次继续。若一源清单已切换而另一源失败，批次必须保持 `degraded`，优先补齐失败源，不得把部分成功展示为完整成功。旧“构建与发布（兼容）”和高级维护中的单源发布只作为受控回退或修复手段。

> 当前新流程已通过 fake provider 和发布器专项自动化测试，但尚未完成真实 GitLink/GitHub 三端双源演练、真实故障恢复演练和人工 UI 对等验收；在这些前置条件完成前继续保留兼容入口。
