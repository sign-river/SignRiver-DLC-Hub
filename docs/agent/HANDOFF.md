...
- 内容要点（标题「1.0.0 正式版已发布」，id `2026-09-22-v1-0-0-release`，日期 2026-09-22）：更新方式＝设置 →「检查更新」；使用方法＝「DLC 库」页选游戏 →「一键解锁」；报错先点「一键修复」→ 不行的去「报错指南」按现象找对应「解决方案」（用户明确要求用这个词，不要写成“排查步骤”）→「报错指南 → 常用工具」里的日志资料收集/补丁工具/杀毒软件检测等可自助排查 → 仍无法解决带截图 +「运行日志」复制的日志到 QQ 群 1061299021。用词已对齐界面真实按钮名（检查更新、一键修复、DLC 库、报错指南、解决方案、常用工具、运行日志）。
