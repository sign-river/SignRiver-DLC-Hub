...
- 更新 `tests/test_publisher_ui_threading.py` 中对应的源码回归断言。
- 验证：`python -m pytest -q tests/test_publisher_ui_threading.py tests/test_ui_theme.py`（127 项通过）；`python -m py_compile src/signriver_publisher/release_center.py`、相关 Ruff、`git diff --check` 通过。未启动发布器 GUI，未构建、未上传、未推送。
- 工作区另有既有未提交改动：`app/versions/0.1.0/app_entry.py`、`tests/test_client_problem_center.py`、`tests/test_ui_theme.py`，未纳入本任务提交。
...
- 已将截图确认的体验问题登记为 `KI-010`：左侧“报错指南”不应显示问题数量；帮助与诊断页底部“仍然无法解决？”诊断导出区域在有垂直余量时不应被压缩。
- 本次仅修改 `docs/known-issues.md` 与本交接文档，未修改运行时代码、缓存或活动模块；未运行 pytest。
...
## 2026-08-23：登记全局清除所有缓存需求

- 已登记 `KI-009`：将“程序与存储”中的“分析并清理”改为“清除所有缓存”。用户确认后的效果为删除应用受控缓存目录内的全部缓存文件，保留“清理当前游戏”作为范围较小的独立操作。
- 当前全局清理仅分析并清理未保护的包缓存、隔离缓存和非活动下载残片，不能清空界面统计的所有缓存。后续实现必须确认缓存根目录边界、模块更新临时文件策略及运行时元数据保留规则，并在下载/安装/补丁/更新任务运行时拦截清理；本轮仅记录，未修改客户端代码、缓存、活动模块、构建或上传。
- 验证（2026-08-23）：读取 `app/state.json` 确认实际活动模块为 `0.2.0`；检查缓存管理 UI 与 `CacheMaintenance` 的现有计划式安全清理逻辑，确认它与“清除所有缓存”语义不同；`git diff --check` 通过。未运行 pytest（仅文档改动）。
## 2026-08-23：登记补丁工具中心需求

- 已登记 `KI-008`：新增当前游戏的“补丁工具”入口，集中展示实际补丁位置、安装/审计状态、原始库备份、平台适用性与云端资源状态，并提供受控打开补丁目录/文件、从云端重新下载补丁等操作。
- 重新下载必须复用现有补丁下载、完整性校验、任务队列和补丁引擎；后续设计需区分下载到缓存和应用到游戏目录，后者必须明确确认且检查游戏未运行。本轮仅记录，未修改客户端代码、资源、活动模块、构建或上传。
- 验证（2026-08-23）：读取 `app/state.json` 确认实际活动模块为 `0.2.0`；检查现有补丁审计、下载任务和应用流程，确认需求记录与现有能力边界一致；`git diff --check` 通过。未运行 pytest（仅文档改动）。

## 2026-08-23：登记安全软件列举与受控打开入口需求

- 已登记 `KI-007`：为 Windows 一键排错增加安全软件检测工具，尽可能列举 Windows Defender/Windows 安全中心、火绒、360 及其他受支持产品；每项通过开发者允许列表提供固定的安全软件或系统安全界面打开入口。用户提到的“艾可菲”具体产品映射待后续确认。
- 需求明确仅做列举和打开入口，不能依据检测结果归因拦截，也不得关闭防护、添加排除项、恢复隔离文件或修改任何安全设置。当前项目只具备疑似拦截启发式与固定 Windows 安全页面限制，本轮仅记录，未修改客户端代码、资源、活动模块、构建或上传。
- 验证（2026-08-23）：检查既有安全软件处理约束和当前活动模块 `0.2.0` 的一键排错能力，确认需求记录与现有安全边界一致；`git diff --check` 通过。未运行 pytest（仅文档改动）。

## 2026-08-23：登记一键排错声明式诊断工具调入需求

- 已登记 `KI-006`：开发者可通过受版本控制的声明注册随程序/模块资源下载的只读诊断工具，一键排错按声明调入并把结构化结果统一呈现，从而复用工具逻辑、避免重复编写 UI 检查代码。用户不应拥有任意工具增删、任意命令编辑或任意文件执行入口。
- 该需求需与既有“一键排错非破坏性”原则对齐：默认只允许审核过的诊断工具；任何会修改游戏、系统或网络设置的能力必须从一键排错中分离并另行确认。当前 `GuideTool` 与固定检查步骤队列尚未关联，本轮仅记录，未修改客户端代码、资源、活动模块、构建或上传。
- 验证（2026-08-23）：检查当前活动模块 `0.2.0` 的 `GuideTool` 定义、下载流程和一键排错固定步骤队列，确认需求记录与现有能力边界一致；`git diff --check` 通过。未运行 pytest（仅文档改动）。
...
- 当前已有 `GuideTool` 数据模型和下载能力，但 UI 下载完成后只打开目录，尚无独立工具中心、详情页、受控卸载、程序内启动或运行结果展示。后续实现必须遵循已确定的平台过滤、HTTPS、非空下载、原子替换和用户确认执行边界；本轮仅记录，未修改客户端代码、不同步活动模块、不构建或上传。
- 验证（2026-08-23）：检查当前活动模块 `0.2.0` 的 `GuideTool`、下载流程和界面入口，确认需求记录与现有能力边界一致；`git diff --check` 通过。未运行 pytest（仅文档改动）。

## 2026-08-23：登记一键排错解决方案返回导航问题

- 已登记 `KI-003`：从一键排错点击“查看解决方案 →”进入详情后，右上角固定显示“返回指南”且只回到解决方案列表；期望该来源显示“回到一键排错”并回到保留检测结果的排错页面。
- 已定位当前导航缺少来源状态：`_open_solution_article()` 直接跳至“常见问题教程”，详情页按钮固定调用 `_show_solution_list()`。本轮仅记录，未修改客户端代码、不同步活动模块、不构建或上传。
- 验证（2026-08-23）：检查当前活动模块 `0.2.0` 的 `app_entry.py` 中一键排错到解决方案详情的跳转与返回绑定，确认问题记录与实现一致；`git diff --check` 通过。未运行 pytest（仅文档改动）。
...
- 新增受 Git 跟踪的 `docs/known-issues.md`，用于集中记录“已确认、暂不立即修复”的产品与开发问题；它与客户端自动生成的运行时“问题记录中心”分工明确，不能相互替代。
- 已登记 `KI-001`：一键排错会把约 110 GB 可用磁盘空间显示为红色。根因已定位为磁盘空间检查无条件附带 `disk-space` 解决方案 ID，渲染层据此统一采用危险色；本轮仅记录，未修改客户端行为、未同步活动模块、未构建或上传。
- 验证（2026-08-23）：检查 `app/versions/0.2.0/app_entry.py` 的一键排错生成与渲染逻辑，确认问题记录与当前实现一致；`git diff --check` 通过。未运行 pytest（仅文档改动），未修改客户端、未构建、未上传、未推送。
...
- 修改范围：`app/versions/0.1.0/app_entry.py`；将“一键修复”从展开的低频操作区移到常用工具栏原“逐项管理 DLC”位置，将“逐项管理 DLC”放入“高级操作”展开区；“更多操作/收起操作”统一改为“高级操作/收起高级操作”。
- 已按功能范围同步到当前活动模块 `app/versions/0.2.0/app_entry.py`，未整目录覆盖；活动版本仍为 `0.2.0`。
- 新增 UI 源码回归断言，验证按钮顺序和文案。
- 验证：`python -m pytest -q tests/test_ui_theme.py`（57 项通过）；`python -m py_compile app/versions/0.1.0/app_entry.py app/versions/0.2.0/app_entry.py` 通过；`git diff --check` 通过。未启动 GUI、未构建、未上传、未推送；客户端若已运行需重启客户端后查看。
...
## 2026-08-26：工具详情剩余空间自适应

- 修复详情上方内容区只使用 `fill="x"` 导致底部出现大片空白的问题；改为 `fill="both", expand=True`，由上方详情区吸收剩余窗口空间。
- 日志工具栏与固定矮日志框仍贴在底部；详情内容较少时不需要滚动，内容较多时仅上方独立区域滚动。
- 已同步活动模块 `0.2.0`；定向测试、两个模块编译、Ruff 与 `git diff --check` 均通过，未执行 GUI 人工验收、构建、上传或推送。

## 2026-08-26：日志控制台面板化与工具栏轻量化

- 日志控制台改为带边框圆角面板：顶栏左侧显示“运行日志”，右侧紧凑排列 `⧉ 复制`、`⌫ 清空` 与“锁定滚屏”。日志文本区作为面板 Body，继续使用等宽字体和独立高度。
...
## 2026-08-26：修复日志面板双层圆角边框残影

- 日志 Card 统一由外层容器承担唯一边框和圆角；Header 改为无圆角、无内缩的顶格区域，Body 文本框去除自身圆角与边框，避免左右角露出“耳朵”以及顶部边框断裂。
- 外层 Card 使用浅灰面板背景，Body 保持白底黑字；已同步活动模块 `0.2.0`。
- 验证：65 项定向测试、两个模块 `py_compile`、Ruff、`git diff --check` 通过；未执行 GUI 人工验收、构建、上传或推送。

## 2026-08-26：日志 Header 与 Body 统一白色卡片风格

- 日志面板外层与 Header 统一使用白色背景，移除灰色 Header 与白色 Body 的强烈分层感。
- 保留一条细分隔线区分工具栏和日志正文，继续由单一外层 Card 承担边框与圆角；已同步活动模块 `0.2.0`。
- 验证：65 项定向测试、两个模块 `py_compile`、Ruff 通过。工作区另有与本任务无关的既有修改，未触碰或暂存这些文件。

## 2026-08-26：修正日志面板顶部圆角露角

- 日志面板外层圆角由 10 调整为 8，顶部 Header 圆角设为 7，与外层边框对齐，消除左右两个像“白球”的圆角露角。
- 已同步活动模块 `0.2.0`；定向测试、编译和 Ruff 检查通过，未执行 GUI 人工验收、构建、上传或推送。
...
## 2026-08-26：清理客户端全部白灰按钮来源

- 全局按钮主题函数的未分类默认分支不再使用白灰 `panel/border` 组合，改为浅蓝次要按钮样式；问题详情“复制”按钮的直接白灰配置也改为 `BUTTON_SECONDARY`。
- 基线与活动模块 `0.2.0` 已同步。验证：相关 pytest 全部通过，两个模块编译、Ruff、`git diff --check` 通过；最后的白灰样式扫描无匹配。未执行 GUI 人工验收、构建、上传或推送。

## 2026-08-26：安全软件检测详情返回按钮移至最右侧
