...
- 现在：配置了更新清单时启动即检查（含 15s/45s 静默重试，失败只写日志不打扰用户）；检测到 `mandatory` 更新后先弹“必须更新”提示（`askokcancel`，关闭提示即退出程序），确认后禁用主窗口（`-disabled`，不支持的平台退化为不提供跳过入口）并**不自动下载**——界面只留一个「下载更新包」按钮、隐藏“取消下载”；点按钮才开始下载安装，安装完成后自动退出重启；下载/安装失败会解锁并提示，下次启动仍会强制检测（避免网络故障把用户彻底锁死）。
