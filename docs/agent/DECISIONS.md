...
- 现在：配置了更新清单时启动即检查（含 15s/45s 静默重试，失败只写日志不打扰用户）；两种更新都先弹自定义对话框（措辞委婉，说明建议先完成更新而不是“必须”）：强制更新只有「立即更新」「退出程序」（关闭窗口等同退出程序），确认后禁用主窗口（`-disabled`，不支持的平台退化为不提供跳过入口）并**不自动下载**——更新区只留一个「下载更新包」按钮、隐藏“取消下载/暂不更新”，点按钮才开始下载安装；非强制更新则有「下载更新包」「暂不更新」两个对话框按钮，关闭后更新区同时提供「下载更新包 / 取消下载 / 暂不更新」三个入口。安装完成后自动退出重启；下载/安装失败会解锁并提示，下次启动仍会强制检测（避免网络故障把用户彻底锁死）。
