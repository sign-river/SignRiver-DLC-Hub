...
## 2026-08-29：优化指南详情页重复打开延迟

- 指南详情页新增按 `article_id` 缓存的正文容器：首次打开仍按原逻辑构建，之后再次打开同一指南只切换已生成的 frame，不重复创建文本框、标签、图片和按钮。
- 保留原有图片点击、工具/指南按钮回调、返回上下文和滚动容器；缓存命中时同步隐藏列表并更新返回按钮，避免导航状态残留。
- 基线 `0.1.0` 已修改并同步到活动版本 `0.2.0`；未修改 `app/state.json`。验证：两个版本 `py_compile`、基线 Ruff、`tests/test_ui_theme.py tests/test_platform_content.py`（全部通过）、`git diff --check`；未执行构建、上传或推送。

## 2026-08-29：修复指南快速滚动重影

- 发现整棵详情控件树缓存与 `CTkScrollableFrame` 内部画布重测冲突：隐藏旧 frame 后仍可能残留 canvas item，快速滚动时出现正文和图片重复叠加。
- 已移除详情控件树缓存，恢复为每次只重建当前正文，并对旧子控件逐个安全 `destroy()`；图片点击、按钮回调、正文视觉组件和滚动容器保持不变。
- 基线与活动版本已同步，客户端已重启，当前源码进程 PID 16952。验证：两个版本 `py_compile`、基线 Ruff、`tests/test_ui_theme.py tests/test_platform_content.py`（全部通过）、`git diff --check`；未执行构建、上传或推送。
...
## 2026-08-29：按当前体验版内容重建首次安装包

- 保持活动版本 `0.2.0` 不变，未递增版本号；按当前源码重新构建模块归档和 Windows 首次安装包。
- 首次安装 ZIP：`dist/唏嘘南溪DLC一键解锁工具-v0.2.0-windows-x64.zip`，22,392,396 bytes，SHA-256 `a7170737300143ca8707c27bd1db039982b82546f26b8f1fbbaf22a791a87da9`。
- 同步生成自解压候选：`dist/唏嘘南溪DLC一键解锁工具-v0.2.0-windows-x64-自解压.exe`；未生成或交付全量更新包，未上传或推送。
- 验证：模块构建成功，ZIP `testzip()` 通过（505 个条目），活动版本读取为 `0.2.0`，`git diff --check` 通过；未执行 GUI 人工验收。
