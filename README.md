...
- **一键修复**：先下载并校验补丁与全部 DLC、完成磁盘空间预检，全部资源就绪后才清理旧内容并立即从缓存重装，最后复检补丁和 DLC；准备失败不会先破坏现有游戏。
- **一键移除补丁**：反向操作，删除补丁 DLL 与 `cream_api.ini`，并把 `steam_api64_o.dll` 还原为 `steam_api64.dll`。
- **移除本程序安装内容**：检查游戏进程、下载任务与原版 DLL 备份后，只撤销有本程序安装凭据的 DLC、恢复被覆盖内容并移除补丁；游戏原有内容、其他来源内容和下载缓存均保留。
