...
class PatchEngine:
    ...
    def audit(
        ...
        *,
        ...
        unlocker = game_root / unlocker_name
        backup = game_root / backup_name
        ini = game_root / ini_name
        matching: list[str] = []
        modified: list[str] = []
        missing: list[str] = []
        ...
        if unlocker_exists:
            if unlocker.stat().st_size == expected_unlocker_size:
                matching.append(unlocker_name)
            else:
                modified.append(unlocker_name)
        else:
            missing.append(unlocker_name)
        if backup_exists:
            if backup.stat().st_size == expected_backup_size:
                matching.append(backup_name)
            else:
                modified.append(backup_name)
        else:
            missing.append(backup_name)
        if ini_exists:
            matching.append(ini_name)
        else:
    ...
    def apply(
        ...
        *,
        ...
        backup_name = self.profile.original_backup_dll_name
        ini_name = self.profile.template.ini_target_name
        unlocker_path = game_root / unlocker_name
        backup_path = game_root / backup_name
        ini_path = game_root / ini_name
        ...
        try:
            ...
            if not backup_path.is_file():
                ...
                if game_backup_size != our_backup_size:
                    ...
                    replaced_paths.append(backup_name)
            ...
            if not unlocker_path.is_file():
                ...
                unlocker_replaced = True
                replaced_paths.append(unlocker_name)
            else:
            ...
            if ini_path.is_file():
                ...
                if existing != ini_payload:
                    ...
                    ini_written = True
                    replaced_paths.append(ini_name)
            else:
                self._write_file_atomic(ini_payload, ini_path, actions)
                ini_written = True
