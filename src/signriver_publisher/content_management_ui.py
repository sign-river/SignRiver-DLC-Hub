...
class ContentManagementUiMixin:
    ...
    def _build_content_release_tab(self) -> None:
        ...
        self.content_release_scope_label = ctk.CTkLabel(
            scope,
            text="新建批次将冻结当前游戏、Release 标签和双源目标；已有批次不会因切换游戏而改变。",
            text_color=MUTED,
            justify="left",
            anchor="w",
            wraplength=760,
        ...
        ctk.CTkButton(
            ...
            text="创建 / 打开本游戏发布批次",
            fg_color=BLUE,
            command=self._create_or_open_game_content_release_batch,
            height=44,
            font=("Microsoft YaHei UI", 14, "bold"),
    ...
    def _build_games_tab(self) -> None:
        ...
        hub_workflow = self._card(self.games_scroll, 1, "Hub / 公告发布")
        ctk.CTkButton(
            hub_workflow,
            text="创建 Hub / 公告快照批次",
            fg_color=LIGHT_BLUE,
            command=self.create_hub_release_batch,
