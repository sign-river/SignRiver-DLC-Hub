...
class AcceptanceManager:
    ...
    def cases_for(self, profile: GameProfile) -> tuple[AcceptanceCase, ...]:
        ...
        if profile.dlc_group_search_roots:
            cases.append(_GROUPED_DLC_CASE)
        return tuple(cases)
...
_COMMON_CASES = (
    ...
    AcceptanceCase(
        ...
        "下载",
        "GitLink 与 GitHub 下载源切换",
        "确认两种下载源读取同一套卡带协议，切换后目录、附件和安装状态不会串线。",
        (
            "确认 GitLink 与 GitHub 的 hub 和当前游戏 Release 均已发布同一批资源。",
            "记录当前下载源、当前游戏和 DLC 数量。",
        ),
        (
            "在没有活动任务时切换到另一下载源并刷新目录。",
            "切回原下载源并再次刷新。",
            "启动一个下载任务后尝试切换下载源。",
        ),
        (
            "两种来源展示相同卡带和 DLC，已安装状态保持一致。",
            "下载源选择在客户端重启后仍然保留。",
            "存在活动任务时切换被阻止，不会让同一任务混用两个来源。",
        ),
        ("恢复发布前计划使用的默认下载源。",),
        "少量网络读取",
    ),
    AcceptanceCase(
        "download.never-timeout",
        "下载",
        "不超时模式",
        "确认极慢或短时中断的网络在开启不超时后可以持续等待，同时取消操作仍然有效。",
        (
            "准备一个下载时间足够观察的 DLC。",
            "使用限速、网络暂停或其他可恢复方式模拟短时无数据。",
        ),
        (
            "保持不超时关闭，观察普通超时与重试行为。",
            "开启不超时后重新下载，短时断网后恢复网络。",
            "等待期间执行一次取消全部下载。",
        ),
        (
            "普通模式仍按既定超时和重试策略处理。",
            "不超时模式不会仅因长时间无数据而判定失败，网络恢复后能继续完成。",
            "不超时不会屏蔽用户主动取消，程序可以正常退出。",
        ),
        ("关闭不超时并恢复正常网络。",),
        "中等下载",
    ),
    ...
    AcceptanceCase(
        "dlc.selection-after-refresh",
        "DLC",
        "刷新后默认全选",
        "确认每次目录刷新都会开始新的选择会话，避免用户点击一键解锁后因没有勾选而无响应。",
        (
            "准备同时包含已安装和未安装 DLC 的目录。",
            "在简洁视图中先手动取消若干选择。",
        ),
        (
            "点击刷新目录。",
        ...
        (
            "每次成功刷新后，当前可选择的 DLC 均默认勾选。",
            "已安装项仍保持正确的禁用或已安装状态。",
            "全选/取消全选按钮文字与当前选择状态一致。",
        ),
        ("无需额外恢复；按后续测试需要调整选择。",),
    ),
    AcceptanceCase(
        ...
        "补丁",
        "补丁测试环境",
        "集中检查补丁文件、记录可恢复基线，并按需构建客户端异常处理场景。",
        ("选择实际游戏目录并确认当前补丁文件处于可恢复状态。",),
        (
            "检查并记录当前补丁环境。",
            "记录补丁基线后，选择需要验证的失败场景并构建测试环境。",
            "完成客户端验证后恢复测试环境。",
        ),
        (
            "环境检查结果已留档。",
    ...
    AcceptanceCase(
        "patch.final-integrity",
        "补丁",
        "DLC 阶段补丁完整性复检",
        "确认补丁在 DLC 下载或安装期间被删除、隔离或篡改时，最终流程不会误报一键解锁成功。",
        (
            "完整备份当前补丁目录。",
            "选择一个下载时间足够进行人工操作的 DLC。",
        ),
        (
            "执行一键解锁并等待补丁应用完成、DLC 开始下载。",
            "在 DLC 下载期间临时移走或隔离当前补丁 DLL。",
            "等待本轮 DLC 任务结束并检查弹窗、状态和日志。",
        ),
        (
            "流程结束前会重新核对补丁文件。",
            "补丁缺失或不一致时明确提示一键解锁未完整完成，不会只因 DLC 安装完成而弹出成功。",
            "已经完整安装的 DLC 保持可识别，不留下半套补丁事务。",
        ),
        ("从备份恢复补丁目录，或重新执行一键解锁完成修复。",),
        "中等下载",
    ),
    AcceptanceCase(
        ...
        "恢复",
        "移除本程序安装内容",
        "确认该操作只撤销本程序有安装记录的内容，并恢复可信原版 DLL。",
    ...
    AcceptanceCase(
        ...
        "恢复",
        "一键修复完整重建",
        "验证一键修复先完整准备和校验资源，再清理目标内容并从缓存重建完整状态。",
        ("确认网络、磁盘空间和测试时间足够。", "记录游戏目录和缓存状态。"),
        ("点击一键修复并阅读大流量提示。", "确认资源准备阶段不会先删除游戏文件。", "等待补丁与全部 DLC 完成。"),
    ...
    AcceptanceCase(
        "ui.remote-announcement",
        "界面",
        "远程公告更新与静默",
        "确认客户端能读取 hub 公告，并以公告版本 ID 正确处理“下次更新前不再显示”。",
        (
            "在发布器的卡带管理中准备一条测试公告并记录原公告。",
            "使用新的公告版本 ID 发布 hub。",
        ),
        (
            "启动客户端并查看公告。",
            "选择“下次公告更新前不再显示”，关闭后重启客户端。",
            "保持正文不变但更换公告版本 ID，再次发布 hub 并重启客户端。",
        ),
        (
            "新公告首次启动时正常展示，标题、日期和正文完整。",
            "同一版本 ID 被静默后不再重复弹出。",
            "版本 ID 更新后公告重新显示；远程公告暂时不可用时客户端仍能正常进入主界面。",
        ),
        ("恢复正式公告或在发布器中停用测试公告，并恢复公告显示设置。",),
        "少量网络读取",
    ),
)
...
_GROUPED_DLC_CASE = AcceptanceCase(
    "dlc.grouped-directory-layout",
    "DLC",
    "多路径聚合 DLC 安装与修复",
    "验证一个逻辑 DLC 分布在多个卡带声明路径时，安装、识别、移除和一键修复都按同一份路径映射执行。",
    (
        "选择卡带中存在于多个聚合分支的同名 DLC。",
        "记录各分支安装前的目录和文件状态。",
    ),
    (
        "安装该 DLC 并刷新目录。",
        "关闭并重启客户端，确认仍能识别。",
        "执行移除本程序安装内容，随后重新安装并执行一键修复。",
    ),
    (
        "资源包中的每个相对路径均落到卡带声明的对应分支，没有被错误平铺。",
        "任一必要分支缺失时不会误判为完整安装。",
        "移除只撤销本程序记录的聚合路径；一键修复能从缓存或下载完整重建全部分支。",
    ),
    ("按需要保留修复后的完整 DLC，或使用移除本程序安装内容撤销本轮测试。",),
    "中等或大量下载",
)
