...
class GitLinkCli:
    ...
    def repository_info(self, repository: GitLinkRepository) -> dict[str, object]:
        return self._json("repo", "+info", "--owner", repository.owner, "--repo", repository.name)

    def create_repository(self, repository: GitLinkRepository, description: str) -> dict[str, object]:
    ...
    def list_releases(self, repository: GitLinkRepository, *, token: str | None = None) -> dict[str, object]:
        return self._json("release", "+list", "--owner", repository.owner, "--repo", repository.name, token=token)

    def create_release(self, repository: GitLinkRepository, *, tag: str, name: str, body: str, attachment_ids: list[str], token: str | None = None) -> dict[str, object]:
        return self._json(
            "release", "+create", "--owner", repository.owner, "--repo", repository.name,
            "--tag", tag, "--name", name, "--body", body,
            "--attachment-ids", ",".join(attachment_ids),
            token=token,
        )

    def update_release(self, repository: GitLinkRepository, *, release_id: str, tag: str, name: str, body: str, attachment_ids: list[str], token: str | None = None) -> dict[str, object]:
        return self._json(
            "release", "+update", "--owner", repository.owner, "--repo", repository.name,
            "--id", release_id, "--tag", tag, "--name", name, "--body", body,
            "--attachment-ids", ",".join(attachment_ids),
            token=token,
        )
    ...
    def _json(self, *arguments: str, token: str | None = None) -> dict[str, object]:
        ...
        try:
            ...
            result = subprocess.run(command, capture_output=True, text=True, encoding="utf-8", errors="replace", timeout=60, check=False, shell=False, env=environment)
...
class GitLinkAttachmentClient:
    ...
    def __init__(self, token: str | None = None, *, base_url: str = "https://www.gitlink.org.cn") -> None:
    ...
    def upload(self, path: Path, progress: Callable[[int, int], None] | None = None) -> str:
        if not self.token:
        ...
        try:
            ...
            if response.status < 200 or response.status >= 300:
                ...
                raise GitLinkError(f"上传 {path.name} 失败（HTTP {response.status}）：{message[:300]}")
            try:
    ...
    def list_releases(self, repository: GitLinkRepository) -> dict[str, object]:
        return self._json_request("GET", f"/api/{repository.owner}/{repository.name}/releases.json?page=1&limit=100")

    def create_release(self, repository: GitLinkRepository, *, tag: str, name: str, body: str, attachment_ids: list[str]) -> dict[str, object]:
    ...
    def update_release(self, repository: GitLinkRepository, *, release_id: str, tag: str, name: str, body: str, attachment_ids: list[str]) -> dict[str, object]:
    ...
    def _json_request(self, method: str, target: str, payload: dict[str, object] | None = None, *, allow_empty: bool = False) -> dict[str, object]:
        ...
        try:
            ...
            if response.status < 200 or response.status >= 300:
                message = raw.decode("utf-8", errors="replace").strip()
                raise GitLinkError(f"GitLink API 请求失败（HTTP {response.status}）：{message[:300]}")
...
def _safe_filename(name: str) -> str:
    return name.replace("\\", "_").replace("/", "_").replace('"', "_").replace("\r", "_").replace("\n", "_")
...
def _release_payload(tag: str, name: str, body: str, attachment_ids: list[str]) -> dict[str, object]:
    return {
        "tag_name": tag,
        "name": name,
        "body": body,
