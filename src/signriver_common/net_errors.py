...
def describe_network_error(
    ...
    *,
    ...
    provided so timeouts and DNS failures are easy to diagnose. Already-described
    messages are returned as-is to avoid double wrapping.
    """
    detail = str(error).strip() or error.__class__.__name__
    if detail.startswith(_KNOWN_HEADS):
        return detail
