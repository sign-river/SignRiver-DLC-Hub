...
def test_latest_installer_tool_uses_selected_source_manifest_without_updater() -> None:
    ...
    assert "如果程序更新时遇到报错" in source
    assert "将压缩包放到任意位置解压" in source
...
def test_paradox_launcher_tools_are_builtin_and_guard_running_process() -> None:
    ...
    assert 'backup = target.with_name("steam_api64_o.dll")' in source
    assert "steam_api64.original.dll" not in source
